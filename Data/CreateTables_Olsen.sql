...
IF OBJECT_ID('SectionInstructor')   IS NOT NULL DROP TABLE SectionInstructor;

-- create CoursePrerequisite table
CREATE TABLE CoursePrerequisite (
    CourseID        INT NOT NULL,
    PrerequisiteID   INT NOT NULL,
    MinGrade         NVARCHAR(2) NULL CHECK (MinGrade IN ('A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'D-', 'F', 'P', 'NP')),
...
CREATE TABLE RegistrationSection (
    RegistrationSectionID  INT NOT NULL,
    LetterGrade            NVARCHAR(2) NULL CHECK (LetterGrade IN ('A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'D-', 'F', 'P', 'NP')),
    StudentRating          INT NULL CHECK (StudentRating BETWEEN 1 AND 5),
    StudentComments        NVARCHAR(MAX) NULL,
    CONSTRAINT PK_RegistrationSection PRIMARY KEY (RegistrationSectionID, SectionID),
    CONSTRAINT FK_RegistrationSection_Registration FOREIGN KEY (RegistrationSectionID)
...
CREATE TABLE AppUser (
    ...
    Password        VARBINARY(256)  NOT NULL,      -- store salted hash
...
CREATE TABLE Student (
    ...
    SemesterGPA decimal(3,2) not null 
        constraint DF_Student_SemesterGPA DEFAULT 0.00,
...
CREATE TABLE Course (
    ...
    CourseTitle     NVARCHAR(10)   NOT NULL,      -- e.g., 'MIST'
    CourseNumber    NVARCHAR(10)   NOT NULL,      -- e.g., '460'
    Semester        NVARCHAR(10)   NOT NULL,      -- e.g., 'Fall'
    CourseDescription     NVARCHAR(MAX)  NULL,
    CreditsHours         DECIMAL(4,1)   NOT NULL CONSTRAINT DF_Course_Credits DEFAULT (3.0),
	Capacity int not null default(0),
    RemainingSeats int not null default(0),
    Year int not null,
    CONSTRAINT UK_Course_SubjectNumber UNIQUE (CourseTitle, CourseNumber),
    CONSTRAINT CK_Course_Credits CHECK (CreditsHours > 0 AND CreditsHours <= 12.0)
);
GO
...
CREATE TABLE Section (
    ...
    SectionTime          NVARCHAR(12) NOT NULL,
    SectionDate          NVARCHAR(12) NOT NULL,
...
CREATE TABLE SectionInstructor (
    SectionInstructorID INT IDENTITY(1,1) PRIMARY KEY,
    SectionID INT NOT NULL,
    InstructorID INT NOT NULL,
    CONSTRAINT FK_SectionInstructor_Section FOREIGN KEY (SectionID)
        REFERENCES Section(SectionID) ON DELETE CASCADE,
    CONSTRAINT FK_SectionInstructor_Instructor FOREIGN KEY (InstructorID)
